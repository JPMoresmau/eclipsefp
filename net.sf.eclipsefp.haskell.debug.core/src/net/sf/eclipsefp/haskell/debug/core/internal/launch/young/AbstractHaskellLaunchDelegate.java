...
public abstract class AbstractHaskellLaunchDelegate extends
      ...
      try {
        ...
        if( process != null ) {
          if( isBackground( configuration ) ) {
            ...
            Job endJob = new Job(
                NLS.bind( CoreTexts.running, loc.toOSString() ) ) {
  ...
  protected abstract void postProcessCreation(
      final ILaunchConfiguration configuration, final String mode,
      final ILaunch launch, IProcess process ) throws CoreException;

  protected abstract void preProcessCreation(
      final ILaunchConfiguration configuration, final String mode,
      final ILaunch launch, Map<String, String> processAttribs )
      throws CoreException;

  protected abstract void preProcessDefinitionCreation(
      final ILaunchConfiguration configuration, final String mode,
  ...
  private IProcess createProcess( final ILaunchConfiguration configuration,
    ...
    try {
      ...
      if( proc != null ) {
        ...
        process = DebugPlugin.newProcess( launch, proc,
            configuration.getName(), processAttrs );
  ...
  protected String getUserArguments( final ILaunchConfiguration config )
    ...
    String args = config.getAttribute( ILaunchAttributes.ARGUMENTS,
        ILaunchAttributes.EMPTY );
    return extra + " " + args; //$NON-NLS-1$
  }

  protected String getRTSArguments( final ILaunchConfiguration config )
      throws CoreException {
    String rts = config.getAttribute( ILaunchAttributes.RTS_ARGUMENTS,
        ILaunchAttributes.EMPTY );
    return rts;
  }
  ...
  public abstract IPath getExecutableLocation( final ILaunchConfiguration config )
      throws CoreException;
