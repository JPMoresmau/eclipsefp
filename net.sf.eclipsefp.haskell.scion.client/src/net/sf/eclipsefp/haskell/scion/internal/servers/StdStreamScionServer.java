...
public class StdStreamScionServer extends ScionServer {
	...
	public StdStreamScionServer(String projectName, IPath serverExecutable,
			Writer serverOutput, File directory) {
		super(projectName, serverExecutable, serverOutput, directory);
	}

	protected synchronized void doStartServer(String projectName)
		...
		try {
			...
			throw new ScionServerStartupException(
					ScionText.scionServerCouldNotStart_message, ex);
		}
		// Connect to the process's stdout to capture messages
		// Assume that status messages and such will be UTF8 only
		try {
			serverOutStream = new BufferedReader(new InputStreamReader(process
					.getInputStream(), "UTF8"));
			serverInStream = new BufferedWriter(new OutputStreamWriter(process
	 ...
	 * 
	 * This thread reads the scion-server's standard input and error, looking
	 * for input lines that start with {@link StdStreamScionServer#PREFIX}.
	 * Matching lines are sent to
	 * {@link ScionServer#processResponse(JSONObject)}. All input is echoed to
	 * the {@link ScionServer#serverOutput} output stream.
	 */
					...
					if (responseString != null) {
						if (responseString.startsWith(PREFIX)) {
							response = new JSONObject(new JSONTokener(
									responseString.substring(PREFIX.length())));
							outputWriter.addMessage(FROM_SERVER_PREFIX+response.toString());
							
//							Trace.trace(fromServer, "%s", response.toString());
//							serverOutput.write(FROM_SERVER_PREFIX.concat(
//									response.toString())
//									.concat(PlatformUtil.NL));
//							serverOutput.flush();
...
//							serverOutput
//									.write(responseString + PlatformUtil.NL);
//							serverOutput.flush();
						}
					} else {
						outputWriter.addMessage(ScionText.scionServerGotEOF_message);
//						serverOutput.write(ScionText.scionServerGotEOF_message
//								+ PlatformUtil.NL);
//						serverOutput.flush();
//						Trace.trace(fromServer,
//								ScionText.scionServerGotEOF_message);
						stopServer();
					}
...
//						serverOutput
//								.write(ScionText.scionJSONParseException_message
//										+ PlatformUtil.NL);
...
//						serverOutput.flush();
						...
						ScionPlugin.logError(
								ScionText.scionJSONParseException_message, ex);

						stopServer();
//					} catch (IOException ex2) {
//						ScionPlugin.logError(
//								ScionText.scionServerNotRunning_message, ex2);
//						stopServer();
//					}
