...
public class TemplateFileCreationOperation implements IRunnableWithProgress {
  ...
  public TemplateFileCreationOperation( final ModuleCreationInfo info, final InputStream stream, final String fileExtension ) {
    this.info = info;
    this.stream = stream;
  ...
  public void run( final IProgressMonitor monitor ) throws InvocationTargetException {
    ...
    try {
      ...
      if (generatedFile==null){
        generatedFile = new TemplateFileGenerator( stream, fileExtension ).createFile( monitor, info );
        onlyAdd=true;
      }
        ...
        if (onlyAdd){
         ...
         List<PackageDescriptionStanza> pdss=new ArrayList<>( pd.getStanzas() );
          for (PackageDescriptionStanza pds:pdss){
            if (pds.getType()!=null && pds.isSourceStanza()){
