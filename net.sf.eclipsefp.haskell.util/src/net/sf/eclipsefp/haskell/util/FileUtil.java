...
public class FileUtil {
	...
	/**
	 * Makes the given base name into a valid executable file name. On Unix,
	 * this returns the base name itself. On Windows, it appends ".exe".
	 */
	...
	/**
	 * Returns true if the given file is executable. On Unix, this checks the
	 * file permission bits. On Windows, it checks whether the file extension is
	 * that of an executable file.
	 */
	...
	private static boolean isExecutableUnix(final File file) {
		...
		try {
			Process process = Runtime.getRuntime().exec(
					new String[] { "test", "-x", file.getAbsolutePath() }); //$NON-NLS-1$ //$NON-NLS-2$
	...
	public static File findInPath(final String shortFileName,
			final FileFilter ff) {
